...
function AccountContent() {
  ...
  useEffect(() => {
    fetch('/api/vip/store-info')
      .then((r) => r.json())
      .then((d) => {
        if (d.vip_store_url) setStoreUrl(d.vip_store_url);
      })
      .catch(() => {});
  }, []);
  ...
  const loadDownloads = (authToken: string) => {
    fetch("/api/downloads", { headers: { Authorization: `Bearer ${authToken}` } })
      .then((r) => {
        if (r.status === 403) {
  ...
  const handleRedeemVip = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        setRedeemResult({
          success: true,
          message: data.message || "VIP Subscription successfully activated!",
        });
        setVipCodeInput("");
        setIsVip(true);
        if (refreshUser) {
        ...
        setRedeemResult({
          success: false,
          message: data.error || "Invalid VIP code. Please verify and try again.",
        });
      }
    } catch (err: any) {
      setRedeemResult({
        success: false,
        message: err.message || "Network error while redeeming code.",
      });
    } finally {
  ...
  const getVipExpiryInfo = () => {
    if (!user?.is_vip) return null;
    if (!user?.vip_expires_at) {
      return { label: "Lifetime VIP Access", daysLeft: null, isLifetime: true, expired: false };
    }
    ...
    return {
      label: `Expires on ${exp.toLocaleDateString(undefined, { day: 'numeric', month: 'short', year: 'numeric' })}`,
      daysLeft,
      isLifetime: false,
      expired: diffTime <= 0,
    };
  ...
  const vipInfo = getVipExpiryInfo();
  ...
  return (
    <div className="min-h-screen bg-[#000000] pt-6 pb-24">
      <div className="container mx-auto px-4 space-y-6">
        ...
        {/* VIP Subscription & Redeem Code Card */}
        <div className="bg-gradient-to-br from-[#1c1810] via-[#141519] to-[#141519] border border-amber-500/25 rounded-2xl p-5 sm:p-7 shadow-2xl relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            ...
            <div className="space-y-2 max-w-xl">
              <div className="flex items-center gap-2.5 flex-wrap">
                ...
                <h2 className="text-xl font-black text-white">VIP Membership & Pass</h2>
                {isVip ? (
                  <span className="inline-flex items-center gap-1 bg-amber-500/20 text-amber-300 border border-amber-500/40 text-xs font-bold px-2.5 py-0.5 rounded-full">
                    <Sparkles size={12} /> Active VIP
                  </span>
                ) : (
                  <span className="inline-flex items-center gap-1 bg-white/5 text-gray-400 border border-white/10 text-xs font-medium px-2.5 py-0.5 rounded-full">
                    Free Member
                  </span>
                )}
              </div>
              ...
              <p className="text-xs sm:text-sm text-gray-300 leading-relaxed">
                {isVip ? (
                  <>
                    Your account has active VIP privileges! Enjoy direct high-speed episode downloads, priority 4K/1080p servers, and zero ads.{' '}
                    {vipInfo && (
                      <span className="text-amber-400 font-semibold block sm:inline mt-1 sm:mt-0">
                        • {vipInfo.label} {vipInfo.daysLeft !== null ? `(${vipInfo.daysLeft} days remaining)` : ''}
                      </span>
                    )}
                  </>
                ) : (
                  <>
                    Unlock <span className="text-amber-400 font-semibold">direct episode downloads</span>, high-speed 1080p/4K servers, and an ad-free viewing experience with 1-Month VIP subscription.
                  </>
                )}
              </p>
            </div>

            {/* Right Action: Buy VIP Button */}
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 shrink-0">
              <a
                href={storeUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center justify-center gap-2 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-slate-950 px-5 py-2.5 rounded-xl font-black text-xs sm:text-sm transition-all shadow-lg shadow-amber-500/20 hover:scale-[1.02] active:scale-[0.98]"
              >
                <ShoppingBag size={16} />
                <span>Buy VIP Code</span>
          ...
          <div className="mt-5 pt-5 border-t border-white/5 relative z-10">
            <form onSubmit={handleRedeemVip} className="flex flex-col sm:flex-row items-center gap-3">
              <div className="relative w-full sm:max-w-md">
                ...
                <input
                  ...
                  placeholder="Enter VIP Code (e.g. VIP-XXXX-XXXX-XXXX)"
                  className="w-full bg-[#1e1e24] border border-amber-500/30 rounded-xl pl-10 pr-4 py-2.5 text-xs sm:text-sm text-white placeholder-gray-500 focus:outline-none focus:border-amber-400 transition font-mono tracking-wider uppercase"
                />
              </div>
              ...
              <button
                ...
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-slate-950 px-5 py-2.5 rounded-xl font-bold text-xs sm:text-sm transition-all shadow disabled:opacity-50 disabled:pointer-events-none"
              >
                {isRedeeming ? (
                  <>
                  ...
                  <>
                    <Ticket size={16} />
                    <span>Claim / Redeem VIP</span>
                  </>
                )}
              </button>

              <span className="text-[11px] text-gray-400 hidden lg:inline">
                Codes are single-use, 30 days validity, and stack onto active subscriptions.
              </span>
            </form>

            {/* Redeem Feedback Alert */}
            {redeemResult && (
              <div
        ...
        <div className="flex items-center gap-2 border-b border-white/10 pb-3 overflow-x-auto">
          ...
          >
            ...
            {isVip && downloads.length > 0 ? (
              <span className="bg-black/30 text-white text-[10px] px-1.5 py-0.5 rounded-md font-mono">
                {downloads.length}
              </span>
            ) : !isVip ? (
              <span className="bg-amber-500/20 text-amber-300 text-[10px] px-1.5 py-0.5 rounded font-bold flex items-center gap-0.5">
                <Lock size={10} /> VIP
              </span>
            ) : null}
          </button>
        ...
        {loadingData ? (
          ...
          <div className="space-y-4">
            ...
            {!isVip ? (
              /* VIP Upgrade Banner */
              <div className="bg-gradient-to-br from-amber-500/10 via-[#141519] to-orange-500/10 border border-amber-500/30 rounded-2xl p-8 text-center space-y-4 shadow-xl">
                <div className="w-16 h-16 rounded-2xl bg-amber-500/15 text-amber-400 border border-amber-500/30 flex items-center justify-center mx-auto shadow-inner">
                  <Crown size={30} />
                </div>
                <h3 className="text-xl font-black text-white">VIP Members Only</h3>
                <p className="text-sm text-gray-300 max-w-md mx-auto leading-relaxed">
                  Downloads are exclusively available for VIP members. Buy a VIP code or enter your 1-month code above to unlock high-speed direct downloads for all episodes.
                </p>
                <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-slate-950 px-5 py-2.5 rounded-xl font-bold text-xs transition-all shadow-lg shadow-amber-500/20"
                  >
                    <ShoppingBag size={14} />
                    <span>Buy VIP Code</span>
