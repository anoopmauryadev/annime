import { getEpisodeById, getServersByEpisode, getDownloadsByEpisode, updateEpisode } from "@/lib/db";
...
export async function PUT(request: Request, ctx: RouteContext<'/api/admin/episodes/[id]'>) {
  try {
    ...
    if (thumbFile && thumbFile.size > 0) data.thumbnail = await saveUploadedFile(thumbFile);
