...
interface TranscodeOptions { inputPath: string; outputDirName: string; serverId?: number }
export async function startHlsTranscoding({inputPath,outputDirName,serverId}: TranscodeOptions): Promise<string> {
  ...
  createTranscodeJob({server_id:serverId || null,status:"pending",progress_text:"Queued: 360p, 480p, Original Quality",
    input_path:inputPath,output_dir_name:outputDirName,master_url:master});
